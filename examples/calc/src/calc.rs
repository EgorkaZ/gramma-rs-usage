...
let dfa = {
    let states = vec![
        ...
        (Some(UnitId(8)), vec![]),
        (Some(UnitId(3)), vec![]),
        (Some(UnitId(12)), vec![(5, 6),]),
        (Some(UnitId(10)), vec![]),
        (None, vec![(7, 7),(2, 0),(4, 1),(3, 5),(0, 3),(6, 2),(8, 4),(5, 6),]),
    ];
    let start_state = 8;
    let symbols = vec![
...
let named_units = vec![
    ...
    (String::from(":PseudoToken:"), UnitId(14)),
    (String::from("EOI"), UnitId(1)),
    (String::from("Dot"), UnitId(13)),
...
let nterms = vec![
    (UnitId(2), vec![RuleId(0),RuleId(1),RuleId(2),RuleId(3),], String::from("f64"), true),
    (UnitId(4), vec![RuleId(4),RuleId(5),RuleId(6),], String::from("f64"), false),
    (UnitId(7), vec![RuleId(7),RuleId(8),], String::from("f64"), false),
    (UnitId(9), vec![RuleId(9),RuleId(10),], String::from("f64"), false),
];
let rules = vec![
    ...
    (UnitId(2), vec![UnitId(5),UnitId(4),], vec![None,Some(String::from("sub")),], String::from(r##"{ -sub }"##)),
    ...
    (UnitId(9), vec![UnitId(12),UnitId(13),UnitId(12),], vec![Some(String::from("f")),None,Some(String::from("s")),], String::from(r##"{ format!("{f}.{s}").parse().unwrap() }"##)),
    (UnitId(9), vec![UnitId(12),], vec![Some(String::from("n")),], String::from(r##"{ n.parse().unwrap() }"##)),
];
    ...
    fn run_action(args: Vec<Self>, rule_id: RuleId) -> Self
    {
        ...
        match rule_id {
            ...
            RuleId(3) => match (args.next().unwrap(),args.next().unwrap()) {
                (Self::_Token(_0),Self::_Term(_1)) => Self::_Expr(_Expr_3(_0,_1)),
                _ => panic!("Invalid rule/argument set: {rule_id:?}"),
            },
            ...
            RuleId(7) => match args.next().unwrap() {
                Self::_Float(_0) => Self::_Factor(_Factor_0(_0)),
                _ => panic!("Invalid rule/argument set: {rule_id:?}"),
            },
            ...
            RuleId(9) => match (args.next().unwrap(),args.next().unwrap(),args.next().unwrap()) {
